...
PROFILE_DXVK = "DXVK"
PROFILE_VKD3D = "VKD3D-Proton"

PROFILE_LABELS = {
    PROFILE_DXVK: "DXVK (DX11 -> Vulkan)",
    PROFILE_VKD3D: "VKD3D-Proton (DX12 -> Vulkan)",
}

VKD3D_GLOBAL_MIP_LOD_BIAS_KEY = "VKD3D_GLOBAL_MIP_LOD_BIAS"

PROFILE_MARKERS = {
    PROFILE_DXVK: {"dxvk.conf", "d3d11.dll", "dxgi.dll"},
    PROFILE_VKD3D: {"d3d12.dll", "d3d12core.dll", "dxgi.dll", "vkd3d-proton.conf"},
}
...
def _looks_like_asset_dir(path: Path, profile: str | None = None) -> bool:
    if not path.exists() or not path.is_dir():
        return False
    if not any(path.iterdir()):
        return False
    names = {p.name.lower() for p in path.iterdir() if p.is_file()}
    if profile in PROFILE_MARKERS:
        markers = PROFILE_MARKERS[profile]
        return bool(names & markers)
    return bool(names & {"dxvk.conf", "d3d11.dll", "d3d12.dll", "dxgi.dll"}) or bool(names)


def _find_asset_directory_in_root(root: Path, profile: str) -> Path | None:
    if not root.exists():
        return None

    profile_folder_names = {
        PROFILE_DXVK: ["dxvk", "dx11", "vulkan-dx11"],
        PROFILE_VKD3D: ["vkd3d-proton", "vkd3d", "dx12", "vulkan-dx12"],
    }

    # Preferred layout: assets/<profile-name>/...
    assets_root = (root / ASSETS_ROOT_REL).resolve()
    for folder_name in profile_folder_names.get(profile, []):
        candidate = (assets_root / folder_name).resolve()
        if _looks_like_asset_dir(candidate, profile):
            return candidate
    ...
    if _looks_like_asset_dir(assets_dir, profile):
        return assets_dir

    # Onefile bundle extraction may put the payload in a temp folder that is not directly
    # named "assets"; scan for the key profile files anywhere under the runtime root.
    for marker in PROFILE_MARKERS.get(profile, set()):
        matches = list(root.rglob(marker))
        if matches:
            return matches[0].parent

    if any((root / name).exists() for name in PROFILE_MARKERS.get(profile, set())):
        return root

    return None


def resolve_assets_root(profile: str) -> Path | None:
    ...
    for root in roots:
        found = _find_asset_directory_in_root(root, profile)
...
def copy_vkd3d_conf(src: Path, dst_dir: Path):
    dst_dir = _register_temp_asset_dir(dst_dir)
    dst_dir.mkdir(parents=True, exist_ok=True)
    dst_conf = dst_dir / "vkd3d-proton.conf"
    if src.exists():
        shutil.copy2(src, dst_conf)
    else:
        dst_conf.write_text(
            "# Generated by Black Desert Vulkan Utility\n"
            "# Used by custom VKD3D-Proton fork with global mip bias support.\n"
            f"{VKD3D_GLOBAL_MIP_LOD_BIAS_KEY}=0.0\n",
            encoding="utf-8",
        )
    return dst_conf
...
def ensure_vkd3d_conf(source_path: Path) -> Path:
    """Return the target vkd3d-proton.conf path, creating a default file when absent."""
    candidate = source_path / "vkd3d-proton.conf"
    if candidate.exists():
        return candidate

    matches = list(source_path.rglob("vkd3d-proton.conf"))
    if matches:
        return matches[0]
...
def patch_vkd3d_global_mip_lod_bias(file_path: Path, bias: float):
    """Update the custom VKD3D global mip bias key while preserving other options."""
    if not file_path.exists():
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(
            "# Generated by Black Desert Vulkan Utility\n"
            "# Used by custom VKD3D-Proton fork with global mip bias support.\n"
            f"{VKD3D_GLOBAL_MIP_LOD_BIAS_KEY}={float(bias)}\n",
            encoding="utf-8",
        )
        return
    ...
    for idx, line in enumerate(lines):
        if re.match(r"^\s*#?\s*VKD3D_GLOBAL_MIP_LOD_BIAS\s*=", line):
            lines[idx] = f"{VKD3D_GLOBAL_MIP_LOD_BIAS_KEY}={float(bias)}"
            updated = True

    if not updated:
        lines.append(f"{VKD3D_GLOBAL_MIP_LOD_BIAS_KEY}={float(bias)}")

    file_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def ensure_source_for_mode(profile: str) -> str | None:
    """Resolve the active asset directory for the selected translation layer."""
    if BUNDLED:
        bundled_src = resolve_assets_root(profile)
        if bundled_src is None or not bundled_src.exists():
            qt_info(
                "Source Missing",
                f"No embedded or sidecar {profile} files were found under 'assets'.\n\nSelect the source folder manually.",
            )
            chosen = qt_directory_prompt(
                f"Select the SOURCE folder for {profile} (files to manage)")
        ...
        if not copied:
            if profile == PROFILE_DXVK:
                src_conf = ensure_dxvk_conf(bundled_src)
                copy_dxvk_conf(src_conf, tempdir)
            else:
                src_conf = ensure_vkd3d_conf(bundled_src)
                copy_vkd3d_conf(src_conf, tempdir)
        log.debug(
            f"[ASSETS] Copied temp {profile} payload -> {tempdir} ({len(copied)} files)")
        return str(tempdir)

    target_dir = resolve_assets_root(profile) or SOURCE_ROOT
    if not target_dir.exists() or not any(target_dir.rglob("*")):
        qt_info(
            "Source Missing",
            f"Default source for {profile} not found or empty:\n{target_dir}\n\nPlease select the source folder manually.",
        )
        chosen = qt_directory_prompt(
            f"Select the SOURCE folder for {profile} (files to manage)")
        return chosen or None
    ...
    if not copied:
        if profile == PROFILE_DXVK:
            ...
            src_conf = ensure_vkd3d_conf(target_dir)
            copy_vkd3d_conf(src_conf, tempdir)
    log.debug(
        f"[ASSETS] Copied temp {profile} payload from source -> {tempdir} ({len(copied)} files)")
    return str(tempdir)


def ensure_source_for_profile(profile: str, lod_bias: float) -> str | None:
    """Build a source directory for the selected profile and patch DXVK LOD bias when needed."""
    source = ensure_source_for_mode(profile)
    if source is None:
        return None
    ...
    elif profile == PROFILE_VKD3D:
        vkd3d_conf = ensure_vkd3d_conf(source_path)
        patch_vkd3d_global_mip_lod_bias(vkd3d_conf, lod_bias)
...
def normalize_install_path(path: str | os.PathLike[str]) -> str:
    try:
        ...
        if candidate.exists():
            try:
                # Resolve existing paths to keep filesystem-preferred casing.
...
def install_path_key(path: str | os.PathLike[str]) -> str:
    """Create a comparison key that is case-insensitive on Windows."""
        ...
        try:
            for line in cache_file.read_text(encoding="utf-8").splitlines():
                ...
                if p and (Path(p) / GAME_EXE).exists():
                    normalized = normalize_install_path(p)
                    key = install_path_key(normalized)
                    if key in seen_keys:
                        continue
                    seen_keys.add(key)
...
def create_lod_bias_controls(parent_widget):
    ...
    title = QLabel("Global sampler LOD bias:")
    title.setStyleSheet("font-size: 11pt; font-family: 'Segoe UI';")
    layout.addWidget(title)

    desc = QLabel(
        "Positive = less detail (more aggressive mip selection)\n"
        "Negative = more detail (sharper textures / higher detail mips)\n"
        "Applied to DXVK and custom VKD3D-Proton fork builds that support VKD3D_GLOBAL_MIP_LOD_BIAS.")
...
def select_installs_dialog(paths):
    ...
    slider, spinbox = create_lod_bias_controls(layout)
    ...
    warning_label = QLabel(
        ...
        "The game must be closed before any translation-layer files are changed."
    )
    ...
    profile_box.addItem(PROFILE_LABELS[PROFILE_DXVK], PROFILE_DXVK)
    profile_box.addItem(PROFILE_LABELS[PROFILE_VKD3D], PROFILE_VKD3D)
    layout.addWidget(QLabel("Translation layer:"))
    layout.addWidget(profile_box)

    lod_hint_label = QLabel(
        "Hint: LOD bias handling in this utility is custom-tuned. "
        "Stock upstream DXVK and stock upstream VKD3D-Proton do not provide the same behavior level used here."
    )
    lod_hint_label.setWordWrap(True)
    lod_hint_label.setStyleSheet(
        "color: #5c4b00; font-size: 9pt; font-family: 'Segoe UI';"
    )
    layout.addWidget(lod_hint_label)
    ...
    state = {"mode": None, "selected": [],
             "lod_bias": 0.0, "profile": PROFILE_DXVK}
    ...
    def set_mode(mode):
        ...
        state["profile"] = str(profile_box.currentData())
        dialog.accept()
    ...
    if dialog.exec() != QDialog.DialogCode.Accepted:
        return (None, [], 0.0, PROFILE_DXVK)

    return (state["mode"], state["selected"], state["lod_bias"], state["profile"])
...
def main():
    ...
    # 4) Selection loop
    while True:
        mode_action, selected, lod_bias, profile = select_installs_dialog(
            installs)
        ...
        source = ensure_source_for_profile(profile, float(lod_bias))
        if not source:
            continue
        log.debug(
            f"[MAIN] Source = {source} | profile = {profile} | samplerLodBias = {lod_bias}")

        if mode_action in ("COPY", "REMOVE"):
            if not ensure_game_closed_for_mutation("importing translation-layer files" if mode_action == "COPY" else "removing translation-layer files"):
                continue
